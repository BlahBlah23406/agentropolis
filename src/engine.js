// Agentropolis: Edition — the rebel-base engine.
...
//   Comms Scriptorium    -> drafting/writing (your-model-name)
...
export const DEPARTMENTS = {
  ...
  research: { id: 'research', name: 'Holocron Library', emoji: '\u{1F4DA}', row: 0, col: 2, workers: 2, duration: 2.2,
    desc: 'Consults the holocrons: looks things up, gathers intel. Real system: your-model-name research specialist.',
    role: 'You are the Holocron Library of Command’s rebel base. Gather concise, factual notes on the request. Reply in under 120 words.' },
  ...
  calendar: { id: 'calendar', name: 'Protocol Scheduler', emoji: '\u{1F4C5}', row: 1, col: 0, workers: 2, duration: 1.4,
    desc: 'Scheduling, reminders, appointments. Real system: your-calendar-model tools + cron (Morning Briefing 05:30, Evening Briefing 22:00).',
  ...
  writing: { id: 'writing', name: 'Comms Scriptorium', emoji: '\u{270D}\u{FE0F}', row: 1, col: 1, workers: 2, duration: 2.4,
    desc: 'Drafting, summarizing, polishing transmissions. Real system: your-model-name writing specialist.',
...
export function llmPlanner({ url = 'http://127.0.0.1:11434', model = 'your-model-name', apiKey = null, fetchFn } = {}) {
  const doFetch = fetchFn || fetch;
  const system = 'You are the War Room of Command’s rebel base of AI departments. Departments: '
  ...
  return async (task) => {
    ...
    const endpoint = isChat ? url : `${url.replace(/\/+$/, '')}/api/generate`;
    const headers = {
      'content-type': 'application/json',
      ...(apiKey ? { authorization: `Bearer ${apiKey}` } : {}),
    };
    const promptText = `Mission: ${task.text}`;
    const bodyObj = {
      model,
      system,
      prompt: promptText,
    ...
    try {
      ...
      if (!isChat) {
        const chatEndpoint = `${url.replace(/\/+$/, '')}/api/chat`;
        res = await doFetch(chatEndpoint, {
          method: 'POST',
          headers,
          body: JSON.stringify(bodyObj),
        });
...
// On this machine that's your-model-name via Ollama Cloud.

export function llmWorkers({ url = 'http://127.0.0.1:11434', model = 'your-model-name', apiKey = null, fetchFn } = {}) {
  const doFetch = fetchFn || fetch;
  ...
  workers.dispatch = llmPlanner({ url, model, apiKey, fetchFn });
  return workers;
}
...
function arrive(city, v) {
  ...
  if (v.kind === 'order' || v.kind === 'handoff') {
    ...
    emit(city, `✅ Mission #${task.id} delivered to Command (${task.results.length} department${task.results.length === 1 ? '' : 's'} contributed) — beep boop!`);
    ...
    for (const slot of state.slots) {
      ...
      if (deptId === 'dispatch') {
        ...
        if (task.plan && task.step < task.plan.length) {
          ...
          task.status = 'returning to Command';
          spawnVehicle(city, task, deptId, 'command', 'result');
        }
      }
    }
