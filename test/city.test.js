...
// The cloud-router plugin lives in the host agent system, not in this repo.
// A standalone checkout has no such file, so those assertions are skipped
// rather than failed — matching how schema.test.js guards on the registry.
const ROUTER_PLUGIN = join(OC, 'plugins', 'cloud-router', 'index.mjs');
const hasRouterPlugin = existsSync(ROUTER_PLUGIN);

test('departments.json: valid registry with governor + unique departments', async () => {
  // Read through loadRegistry() so this checks the registry the app actually
  // uses, whether that comes from a host file or the bundled default.
...
test('cloud-router plugin: loads and registers the city + routing hooks', { skip: !hasRouterPlugin }, async () => {
...
test('cabinet protocol: manager prompt names ministers, forbids cabinet on cron runs', { skip: !hasRouterPlugin }, async () => {
